 ...
 *
 * 为什么要单独一次调用，不塞进记忆抽取里：
 * 记忆那份的输出必须是**干净的 markdown 文件**，混进 JSON 会把它弄脏。
 * 而且两者的性质不同——记忆是"一直带着的长期事实"，
 * 这是"有下文没下文的临时跟进"，生命周期只有几天。
 *
 * 挑什么：对方提到的、**有下文可问**的事。
 * 挑不到就返回空数组，这是常态——大部分聊天里没有这种事。
 */
export function buildRecallPrompt({ transcript, existing = [] }) {
  const system = `你要从一段聊天记录里挑出"过几天该回头问问下文"的事。

【什么该挑】
对方提到的、**结果还没出来**的事：
- 在等一个结果（面试有没有回音、快递到没到、成绩出了没）
- 在纠结一个决定（要不要换工作、要不要跟谁摊牌）
- 身体或情绪上的状况（感冒好没好、那个事过去了没）
- 他自己说要做什么（下周去考试、准备开始跑步）

【什么不该挑】
- 已经聊完、有结果的事（问过了就没意思）
- **已经记在"还没问的"清单里的事——不要重复提出来**
- 泛泛的状态（"最近有点累"——这个没有下文可问）
- 你自己说过的事（只挑**对方**的）
- 当天就能问的琐事（那是普通聊天，不用专门记）
- 太私密、问了会让人不舒服的（如果对方明显不想聊，别记）

【怎么写】
- 写成一句**你自己看得懂的提醒**，不是要发给对方的话。
  好："他那个实习面试，说等通知"
  差："请问您的实习面试有结果了吗"
- 20 字以内。
...
【输出】
只输出 JSON，不要任何其他文字：
{"items": [{"text": "他那个实习面试，说等通知", "afterDays": 3}]}

afterDays 是建议隔几天问：
- 在等结果的（面试、快递、成绩）→ 2-3 天
- 在纠结决定的 → 5-7 天
- 身体状况 → 2-3 天

没有值得记的就返回 {"items": []}。**空数组是正常的**，不要硬凑。`

  const user = `【最近的对话】
${transcript}

${existing.length ? `【已经记着、还没问的（不要重复）】\n${existing.map((t) => '- ' + t).join('\n')}\n\n` : ''}请挑出该回访的事。`
...
- **如果上面有"可以回头问问的事"，那是最该优先说的**——
  隔了几天回头问一句下文，比随便找话说自然得多。
